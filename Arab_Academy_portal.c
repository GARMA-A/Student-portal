...
// table term 2
void displayCourseTable1() {
  char *subjects[NUM_SUBJECTS] = {"Calculus II",       "ESP II",
                                  "Communication",     "Advanced Physics",
  ...
  printf("+--------------------------------------------------------------------"
         "-------------------------+\n");
  printf("|      Subject      |       weak 7       |       weak 12       |   "
         "course work     |     U    |\n");
  printf("+--------------------------------------------------------------------"
         "-------------------------+\n");
  for (int i = 0; i < NUM_SUBJECTS; i++) {
    printf("| %-18s| %-18d| %-18d| %-18d|     -       |\n", subjects[i],
           weak7Marks[i], weak12Marks[i], courseworkMarks[i]);
  }
  printf("+--------------------------------------------------------------------"
         "-------------------------+\n");
}
void CourseTablescince2() {
  char *subjects[NUM_SUBJECTS] = {"Calculus I",        "ESP II",
                                  "Communication",     "Advanced Physics",
                                  "Problem Solving",   "Entrepreneurship",
                                  "Discrete Structure"};
  int maxMarks[NUM_SUBJECTS] = {30, 20, 10, 30, 10, 20, 10};
...
void CourseTablescince1() {
  char *subjects[NUM_SUBJECTS] = {"Pre Calculus ", "ESP I",    "Computing",
                                  "Physics",       "Business", "Creativity",
                                  "INF System"};
  int maxMarks[NUM_SUBJECTS] = {30, 20, 10, 30, 10, 20, 10};
  srand(time(NULL));
  int weak7Marks[NUM_SUBJECTS];
...
// gpa less 2
void displayCourseTable3() {
  char *subjects[5] = {"Calculus I", "ESP I", "Advanced Physics",
                       "Problem Solving", "Entrepreneurship"};
...
// table term 1
void displayCourseTable2() {
  char *subjects[NUM_SUBJECTS] = {"Calculus I", "ESP I",    "Computing",
                                  "Physics",    "Business", "Creativity",
                                  "INF System"};
  ...
  for (int i = 0; i < NUM_SUBJECTS; i++) {
    printf("| %-18s| %-18d| %-18d| %-18d|  %-7s  |\n", subjects[i],
           weak7Marks[i], weak12Marks[i], courseworkMarks[i], finalResult[i]);
  }
  printf("+--------------------------------------------------------------------"
         "-------------------------+\n");
}
void displayCourseTable4() {
  char *subjects[7] = {"Calculus I", "ESP I",      "Computing", "Physics",
  ...
  for (int i = 0; i < 7; i++) {
    ...
    if (strcmp(subjects[i], "Calculus I") == 0 ||
        strcmp(subjects[i], "ESP I") == 0) {
...
void attendance() {
  ...
  float attendance[NUM_SUBJECTS] = {
      0.0, 3.3,  6.0, 12.0,
      0.0, 15.0, 9.0}; // Attendance numbers for each subject
  printf("+---------------------------------+\n");
...
void attendancescince1() {
  char *subjects[NUM_SUBJECTS] = {
      " Pre Calculus ", "ESP I", "Information System ", "Physics", "Computing",
  };
  float attendance[NUM_SUBJECTS] = {3.3, 3.3, 15.0, 15.0,
                                    0.0}; // Attendance numbers for each subject
  printf("+---------------------------------+\n");
  printf("|      Subject      |  Attendance |\n");
  printf("+---------------------------------+\n");
  for (int i = 0; i < NUM_SUBJECTS; i++) {
...
void attendance2() {
  char *subjects[NUM_SUBJECTS] = {
      "Calculus II ",     "ESP I",          "Problem solving ",
      " Advance Physics", "Enrepernership",
  };
  float attendance[NUM_SUBJECTS] = {3.3, 3.3, 15.0, 15.0,
                                    0.0}; // Attendance numbers for each subject
  printf("+---------------------------------+\n");
...
void timetable() {
  char *daysOfWeek[] = {"Saturday", "Sunday",    "Monday",
                        "Tuesday",  "Wednesday", "Thursday"};

  char *subjectCodes[NUM_SUBJECTS] = {"EBA1106", "CCS1001", "CNC1401",
                                      "EBA1204", "UNR1407", "UNR1407",
                                      "UNR2101"};
  char *subjects[NUM_SUBJECTS] = {
      "Advanced Physics",     "Discrete Structures", "Entrepreneurship Skills",
      "Calculus II",          "Academic Writing",    "Academic Writing",
      "Communication  Skills"};
  char *roomNumbers[NUM_SUBJECTS] = {
      "Room # A-106", "Room # A-106", "Room # Online", "Room # H",
      "Room # H",     "Room # E",     "Room # A-204"};
  char *timings[NUM_SUBJECTS][2] = {
      {"8:30 AM", "10:30 AM"}, {"10:30 AM", "12:30 PM"},
      {"6:30 PM", "8:30 PM"},  {"10:30 AM", "12:30 PM"},
      {"12:30 PM", "2:30 PM"}, {"2:30 PM", "4:30 PM"},
      {"8:30 AM", "10:30 AM"}};
  char *sections[NUM_SUBJECTS] = {"Lect.", "Lect.", "Lect.", "Lect.",
                                  "Sec.",  "Sec.",  "Lect."};
  int dayCounter = 0;
  printf("+-----------------+------------------+------------------+------------"
         "-------+----------------------------+\n");
  printf("|       Day       |    Course Start  |    Course End    |     code   "
         "       |    Subject                 |\n");
  printf("+-----------------+------------------+------------------+------------"
         "-------+----------------------------+\n");
  for (int i = 0; i < NUM_SUBJECTS; i++) {
...
void timetablescince2() {
  ...
  char *subjects[NUM_SUBJECTS] = {
      "Advanced Physics",     "Discrete Structures", "Entrepreneurship Skills",
      "Calculus I",           "Academic Writing",    "Academic Writing",
      "Communication  Skills"};
  char *roomNumbers[NUM_SUBJECTS] = {
      "Room # A-106", "Room # A-106", "Room # Online", "Room # H",
      "Room # H",     "Room # E",     "Room # A-204"};
  ...
  for (int i = 0; i < NUM_SUBJECTS; i++) {
    printf("| %-16s| %-16s | %-16s | %-16s  | %-26s |\n",
           daysOfWeek[dayCounter], timings[i][0], timings[i][1],
           subjectCodes[i], subjects[i], roomNumbers[i]);
    dayCounter++;
    if (dayCounter == 7) {
      dayCounter = 0;
    }
...
void timetablescince1() {
  char *daysOfWeek[6] = {"Saturday", "Sunday",    "Monday",
                         "Tuesday",  "Wednesday", "Thursday"};

  char *subjectCodes[6] = {"EBA1106", "CCS1001", "CNC1401",
                           "UNR1407", "EBA1204", "UNR2101"};
  char *subjects[6] = {"Pre Calculus ",    "ESP I",
                       "Communication",    "Problem Solving",
                       "Advanced Physics", "Problem Solving"};
  char *roomNumbers[6] = {"Room # A-206", "Room # A-106", "Room # N",
                          "Room # B",     "Room # A-204", "Room # Online"};
  char *timings[6][2] = {{"8:30 AM", "10:30 AM"}, {"6:30 PM", "8:30 PM"},
                         {"12:30 PM", "2:30 PM"}, {"2:30 PM", "4:30 PM"},
                         {"8:30 AM", "10:30 AM"}, {"4:30 PM", "6:30 PM"}};
  char *sections[6] = {"Lect.", "Lect.", "Sec.", "Sec.", "Lab.", "Lect."};
  int dayCounter = 0;
  printf("+-----------------+------------------+------------------+------------"
         "-------+----------------------------+\n");
  printf("|       Day       |    Course Start  |    Course End    |     code   "
...
void timetable2() {
  ...
  char *subjectCodes[6] = {"EBA1106", "CCS1001", "CNC1401",
                           "UNR1407", "EBA1204", "UNR2101"};
  char *subjects[6] = {"Calculus I",       "ESP I",
                       "Communication",    "Problem Solving",
                       "Advanced Physics", "Problem Solving"};
  char *roomNumbers[6] = {"Room # A-206", "Room # A-106", "Room # N",
                          "Room # B",     "Room # A-204", "Room # Online"};
...
void editDegrees() {
  ...
  while (fgets(line, sizeof(line), file) != NULL) {
    if (strstr(line, "Subject:") != NULL && strstr(line, subject) != NULL) {
      ...
      // Similarly, modify prompts and input for Week 12, Coursework, and Final
      // components
    } else {
      fputs(line, tempFile);
    }
